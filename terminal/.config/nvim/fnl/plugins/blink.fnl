...
(fn transform-items [_ items]
  (icollect [_ item (ipairs items)]
    (if item.label
        item
        (h.assoc item :label (or item.insertText item.filterText "?")))))

[{1 :saghen/blink.compat :version "*" :lazy true}
 {1 :saghen/blink.cmp
  :version "*"
  ...
  :opts {:keymap {:keymap {:<C-j> [:select_next :fallback]
         ...
         :completion {:accept {:auto_brackets {:enabled true}}
                      :menu {:border nil
                             ...
                             :draw {:columns [[:kind_icon]
                                              ...
                                              [:kind]
                                              [:source_name]]}}
                      :documentation {:window {:border nil
                                               :scrollbar false
         ...
         :sources {:default [:lsp :path :snippets :buffer :conjure]
                   :transform_items transform-items
